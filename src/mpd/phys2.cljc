...
(defn mass2
  [x y radius weight friction elasticity]
...
(defn dguard2
  [masses massa massb distance elasticity]
...
(defn aguard2
  [masses massa massb massc minangle maxangle power]
  "create angle guard"
  (let [{wa :w} (masses massa)
        {wc :w} (masses massc)
        sum (+ wa wc)]
...
(defn get-colliding-surfaces-by-distance
  [p d radius surfaces point]
  "collect surfaces crossed by masspoint dir or nearby endpoint"
  (reduce
    (fn [result {t :t b :b :as surface}]
      (let [isp (math2/collide-v2-v2 p d t b radius)]
...
(defn get-intersecting-surfaces
  [p d surfaces]
  "collect surfaces crossed by masspoint dir or nearby endpoint"
  (reduce
    (fn [result {t :t b :b :as surface}]
...
(defn move-mass-back
  [surface-trans surface-dir vector-trans vector-dir radius]
